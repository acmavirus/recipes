...
class Post extends Admin_Controller
{
    ...
    private function __loadTable($page=0)
    {
        ...
        $data['listview'] = $listview = $this->_post->getDataByLM($this->_database, [], 10, $page);
    ...
    public function index()
    {
        ...
        // ==>> START CODE <<== //
        $data['listview'] = $listview = $this->_post->getDataByLM($this->_database, [], 10, 1);
        $fields = $this->db->field_data($this->_database);
        $fields = $this->array_group_by($fields, function ($a) {
            return $a->name;
        });
    ...
    public function edit()
    {
        if (!empty($this->input->post())) {
            ...
            unset($data['id']);
            unset($data['page']);
            if ($this->_post->updateData($this->_database, ['id' => $id], $data)) {
                echo $this->__loadTable($page);
            }
        }
    }
    ...
    public function delete()
    {
        $id = $this->input->post();
        if ($id) {
            $del = $this->_post->deleteDataBy($this->_database, ['id'=>$id['id']]);
            $del2 = $this->_post->deleteDataBy('post_datacrawler', ['post_id'=>$id['id']]);
    ...
    public function getRow()
    {
        $id = $this->input->post();
        if ($id) {
            $del = $this->_post->getDataBy($this->_database, $id);
