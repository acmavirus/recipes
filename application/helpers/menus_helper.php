        ...
        if (!empty($listItem)) foreach ($listItem as $key => $value) {
            ...
            if(count($query->result_array()) > 0) {
                ...
                if (count($query->result_array()) > 0) foreach ($query->result_array() as $k => $v) {
                    $html .= general_root_noChild($v);
                };
                $html .= '</ul></li>';                
            } else {
                $html .= general_root_noChild($value);
            };
        }
        echo $html;
    }

    function general_root_noChild($item) {
        $root_noChild = '<li> <a href="'.base_url($item['url']).'" class="waves-effect waves-dark"> <span class="pcoded-micon"><i class="feather icon-film"></i></span> <span class="pcoded-mtext">';
        $root_noChilde = '</span></a></li>';
        $html = $root_noChild;
        $html .= $item['title'];
        $html .= $root_noChilde;
        return $html;
